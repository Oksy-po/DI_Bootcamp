...
import random
def game(user_number):
    """Compare user's number to a random number between 1 and 100."""
    random_number = random.randint(1, 100)

    if user_number==random_number:
        print("You guessed!Congratulations!")
    elif user_number != random_number:
        print("Sorry, you didn't guess. Maybe next time")
        print(f"Your number is {user_number}. Computer number is {random_number}")
    else:
        print("You should give number from 1 to 100")
...
def make_shirt(size="large",text="I love Python"):
    print(f"Your shirt's size is {size} and your text's size is {text}")


make_shirt()
make_shirt(size="medium")
make_shirt(size="small",text="I love cats")
make_shirt(text="Hello!", size="smaller")
...
def get_random_temp(season):
    """Return a random temperature (in Celsius) appropriate for the given season."""
    if season == "winter":
...
def main():
    ...
    print(f"The temperature right now is {round(temperature, 1)} degrees Celsius.")
